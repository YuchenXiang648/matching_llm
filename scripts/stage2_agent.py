...
def generate_fit_and_question(
    ...
    prompt = (
        "You are writing the Match reflection section for the selected supervisor.\n\n"

        "IMPORTANT SEPARATION RULE:\n"
        "- The supervisor/project facts come ONLY from the Detailed Supervisor Profile in the system message.\n"
        "- The student information below is ONLY for judging fit or mismatch.\n"
        "- Do NOT use the student's interests to describe what the supervisor works on.\n"
        "- If the student's preferred area is not mentioned in the supervisor profile, state the mismatch honestly.\n"
        "- Do NOT force a connection just to make the match sound positive.\n\n"

        "=== STUDENT INFORMATION FOR FIT ASSESSMENT ONLY ===\n"
        f"Student stated interests: {compact_student.get('interest_text', '(not provided)')}\n"
        f"Student stated preferences: {compact_student.get('preference_text', '(not provided)')}\n"
        f"Stage 1 summary of student interests: {stage1_summary if stage1_summary else '(not available)'}\n\n"

        "=== WHAT THE STUDENT SAID DURING STAGE 1 ===\n"
        f"{stage1_memory}\n\n"

        "Now write:\n"
        "1. A brief honest fit analysis based on overlap or mismatch between the student and the selected supervisor profile.\n"
        "2. One guiding question to help the student decide whether they are still interested.\n\n"

        "Strict rules:\n"
        "- Do NOT say the supervisor works on games, NPCs, finance, medicine, VR, or any other student-interest area unless that appears explicitly in the supervisor profile.\n"
        "- If the profile is about a different area, clearly explain that difference.\n"
        "- Keep it concise.\n"
        "- Do not start with meta text like 'Okay, based on...'.\n"
    )
